    ...
    def __init__(self, input_size, C_in, C, n_classes, n_layers, genotype, n_big_nodes, stem_multiplier=4, cell_multiplier=4, spec_cell=False):
        ...
        for i in range(n_layers):
            if i in range(n_layers // 3):
                ...
                self.cells.append(cell)
            if i in [n_layers // 3]:
                self.bigDAG1 = SearchBigDAGPartiallyConnection(n_big_nodes, self.cells, 0, n_layers // 3, stem_multiplier*C_cur, stem_multiplier*C_cur, cell_multiplier * C_cur)
            ...
            if i in range(n_layers // 3 + 1, 2 * n_layers // 3):
                ...
                self.cells.append(cell)
            if i in [2 * n_layers // 3]:
                self.bigDAG2 = SearchBigDAGPartiallyConnection(n_big_nodes, self.cells, n_layers // 3 + 1, 2 * n_layers // 3, C_pp, C_p, cell_multiplier * C_cur)
        ...
        self.bigDAG3 = SearchBigDAGPartiallyConnection(n_big_nodes, self.cells, 2 * n_layers // 3 + 1, n_layers, C_pp, C_p, cell_multiplier * C_cur)
        ...
        for _ in range(3):
            for i in range(self.n_big_nodes):
                ...
                if i < 1:
                    ...
                    self.beta_DAG.append(nn.Parameter(1e-3 * torch.randn(3)))

        self._betas = []
        for n, p in self.named_parameters():
            if 'beta' in n:
                self._betas.append((n, p))
        self.net = SearchStage_PartiallyConnected(input_size, C_in, C, n_classes, n_layers, genotype, self.n_big_nodes, stem_multiplier=stem_multiplier, spec_cell=spec_cell)
...
>>>>>>> 3a91367 (fix: stage-level searchのpartially connectionを修正)
        concat = range(self.n_big_nodes, self.n_big_nodes + 2)
