"""Pytorch Densenet implementation
This file is a copy of https://github.com/pytorch/vision/blob/main/torchvision/models/ 'efficientnet.py'.
In order to learn transitions on the cifar dataset, 
we have added our own functions to make it easier to replace parts of the model.
"""
...
class EfficientNet(nn.Module):
    ...
    def get_classifier(self):
        """ New function. return classifier layer """
        return self.classifier
    
    def get_head(self):
        """ New function, return a part of last sereral layer including classifier layer"""
        head = nn.Sequential(
            OrderedDict(
                [
                    ("head_conv", self.features[8]),
                    ("head_avgpool", self.avgpool),
                    ("head_classifier", self.classifier)
                ]
            )
        )
        return head
