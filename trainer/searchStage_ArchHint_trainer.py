...
from utils.loss import AlphaArchLoss, AlphaLaplacianLoss, HintLoss, KD_Loss, SoftTargetKLLoss
...
class SearchStageTrainer_ArchHintKD(SearchStageTrainer_HintKD):
    ...
    def construct_model(self):
        ...
        self.hint_criterion = AlphaArchLoss().to(self.device)
        # self.hint_criterion = AlphaLaplacianLoss(self.sw, int(self.config.layers/3+2)).to(self.device)
        ...
        # ================= Teacher Model ==================
        # if self.config.teacher_name in teacher_models.darts.__all__:
        #     self.logger.info(f"{self.config.teacher_name} is unsupported teacher model now. H-DAS model is only supported.")
        #     raise NotImplementedError("unsupported teacher model now. H-DAS model is only supported.")
        # 教師モデルと中間層抽出機の定義
        teacher_model, teacher_feature_extractor = self.load_teacher(n_classes)
        # 教師の構造をalphaパラメータに変換する
        ...
        # self.hint_criterion.re__init__(self.model.alpha_DAG)
        # self.model.hint_criterion = self.hint_criterion
        showModelOnTensorboard(self.writer, self.teacher_model, self.train_loader)        
        # ================= Regressor Model ==================
        # 生徒と教師の中間層出力をそろえるRegressorを定義
        info_set = [(14, 14, 128, 128),
                    (7, 7, 256, 256),
                    (7, 7, 512, 512)]
        ...
        self.architect = Architect_Arch(self.model, self.teacher_model, self.config.w_momentum, self.config.w_weight_decay, teacher_feature_extractor=self.teacher_feature_extractor, Regressor=self.Regressor)
    ...
    def load_teacher(self, n_classes):
        ...
        if self.config.teacher_name in teacher_models.darts.__all__:
            feature_extractor = extractor.Feature_Extractor(model)
        else:
    ...
    def train_hint_epoch(self, epoch, printer=print, stage=1):
        ...
        while trn_X is not None:
            ...
            arch_hint_loss = self.architect.unrolled_backward_archhint(trn_X, trn_y, val_X, val_y, cur_lr, self.w_optim, stage=stage)
            ...
            hint_loss = self.model.hint_criterion(self.model.alphas_list(), self.teacher_model.alpha_DAG)
            # hint_loss = self.hard_criterion(student_features_DICT["logits"], trn_y) + self.model.hint_criterion(self.model.alphas_list())
            hint_loss.backward()
